...
void MIDISendContinue(Dubby &dubby)
{
    ...
    void MIDISendNoteOn(Dubby &dubby, uint8_t note, uint8_t velocity)
    {

        if (dubby.dubbyMidiSettings.currentMidiOutOption == MIDIOUT_ON)
        {
            MIDIUsbSendNoteOn(dubby, dubby.dubbyMidiSettings.currentMidiOutChannelOption, note, velocity);
            MIDIUartSendNoteOn(dubby, dubby.dubbyMidiSettings.currentMidiOutChannelOption, note, velocity);
        }
    }

    void MIDISendNoteOff(Dubby &dubby, uint8_t note)
    {

        if (dubby.dubbyMidiSettings.currentMidiOutOption == MIDIOUT_ON)
        {
            MIDIUsbSendNoteOff(dubby, dubby.dubbyMidiSettings.currentMidiOutChannelOption, note);
            MIDIUartSendNoteOff(dubby, dubby.dubbyMidiSettings.currentMidiOutChannelOption, note);
        }
    }
