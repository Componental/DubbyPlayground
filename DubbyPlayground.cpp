...
void handleKnobs()
{
    ...
    */
    ...
    outputVolume = daisysp::fmap(knob4Value, minOutput, maxOutput, Mapping::LOG);
        ...
        knob1Value+=1.f-dubby.GetKnobValue(dubby.CTRL_6)-0.5f;
        knob3Value+=1.f-dubby.GetKnobValue(dubby.CTRL_5)-0.5f;

        float knob1ValueClamped = std::max(std::min(knob1Value,1.f),0.f);
        float knob3ValueClamped = std::max(std::min(knob3Value, 1.f),0.f);
